...
export function VideoEditorControl({ className }: VideoEditorControlProps) {
  ...
  return (
    <div className={cn('video-editor-control bg-background border-border border-t', className)}>
      {/* Timeline */}
      <div className="border-border border-b px-4 py-3">
        <div className="mb-3 flex items-center gap-3">
          ...
          <div
            ref={timelineRef}
            className="bg-muted group relative h-6 flex-1 cursor-pointer rounded"
            onMouseDown={handleMouseDown}
          >
            {/* Timeline background */}
            <div className="from-muted via-muted-foreground/20 to-muted absolute inset-0 rounded bg-gradient-to-r" />
            ...
            {/* Playhead */}
            <div
              className="bg-primary border-background absolute top-1/2 -ml-2 h-4 w-4 -translate-y-1/2 rounded-full border-2 shadow-lg"
              style={{
            ...
            {state.tracks.map((track) => (
              <div key={track.id} className="absolute top-0 h-full">
                {/* Track start */}
                <div
                  className="bg-accent/60 absolute top-0 h-full w-1"
                  style={{ left: `${(track.startTime / state.duration) * 100}%` }}
                />
                {/* Track duration */}
                <div
                  className="bg-accent/30 border-accent/50 absolute top-1 h-4 rounded-sm border"
                  style={{
          ...
          <span className="text-muted-foreground min-w-[60px] font-mono text-sm">{formatTime(state.duration)}</span>
        </div>
      </div>
      ...
      <div className="flex items-center justify-between px-4 py-3">
        ...
        <div className="flex items-center gap-2">
          {/* Skip to start */}
          <Button variant="ghost" size="sm" onClick={() => actions.seekTo(0)} className="h-8 w-8 p-0">
          ...
          {/* Skip to end */}
          <Button variant="ghost" size="sm" onClick={() => actions.seekTo(state.duration)} className="h-8 w-8 p-0">
            <svg className="h-4 w-4" fill="currentColor" viewBox="0 0 24 24">
